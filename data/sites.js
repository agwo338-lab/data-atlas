...
var SITES = [
  ...
  },
  {
    ...
    sources: [
      ...
      { label: "Centersquare — Los Angeles market spec sheet (PDF)", url: "https://www.csquare.com/hubfs/CenterSquare/docs/Centersquare-LAX-SpecSheet.pdf", cls: "P" },
      { label: "Centersquare — Los Angeles data centers page", url: "https://www.csquare.com/data-centers/los-angeles", cls: "P" }
    ],
    provenance: {
      capacityMW: {
        basis: "disclosed",
        asOf: "2026-09-21",
        evidence: [
          { source: 2, cls: "P", note: "Centersquare's own LA spec sheet for LAX3: utility power 24.9MW, UPS capacity 9.2MW, generator capacity 16MW, 73,952 sq ft raised floor. The 9.2MW UPS figure is used — see notes for why, and note the other two are recorded rather than discarded." },
          { source: 3, cls: "P", note: "The same figures on Centersquare's live Los Angeles market page, fetched separately. Same operator and same class, so this corroborates transcription rather than the claim — it is one class however many documents agree." }
        ],
        conflicting: false
      },
      status: {
    ...
    notes: "17836 Gilette Ave, Irvine — Orange County, despite the LAX branding. Centersquare is the 2024 Brookfield rebrand of the merged Cyxtera/Evoque estate, so records for this building may exist under either prior name. Capacity is deliberately null: the MW figures circulating for the LAX sites are directory-grade only, and this project does not take a hard number from a directory. An SCAQMD generator permit or Centersquare's own spec sheet would be the first real figure. Note for anyone reconciling against third-party lists: the \"LAX\" prefix is branding, not geography — LAX3 and LAX5 are in Orange County, and at least one public directory mislabels both as Los Angeles County. BUILDING IDENTITY, and a false lead ruled out (checked 2026-09-21): this is the data center half of \"Irvine Crossings\", a roughly 396,000 sq ft property on about 21 acres addressed as both 17871 Von Karman Ave and 17836 Gillette Ave — one property, two street frontages. The other half, roughly 202,505 sq ft, is leased by Amazon and is a DELIVERY AND DISTRIBUTION facility for same-day and Prime orders, on a lease running to 2027. It is not a data center and there is no AWS data center in Irvine. That distinction matters because at least one directory (datacenter.fyi) scrapes an Amazon building permit at 17871 Von Karman and renders it as \"a operational hyperscale data center with 0.123 MW\" under the permittee AMAZON.COM SERVICES LLC with the internal code SAX1. 0.123 MW is 123kW, which is a single equipment room rather than anything hyperscale, and the record's application and approval dates are both literally 2022-01-01, a placeholder. AMAZON.COM SERVICES LLC is Amazon's general operating entity and appears on fulfilment, delivery and office permits too, so the name alone proves nothing about what a building does. The data center portion, roughly 193,168 sq ft, was leased to Cyxtera through 2029 — and Cyxtera is exactly what became Centersquare in the 2024 Brookfield rebrand, which is why this entry and that lease describe the same space. Unrelated: Amazon did close an Irvine facility in Nov 2024, but that was a 45,000 sq ft warehouse at 2006 McGaw Ave, not this building. For completeness, AWS's only Direct Connect location in all of Southern California is CoreSite LA1 at One Wilshire (AWS code CSOW), which this atlas tracks as coresite-onewilshire-la1 — SoCal AWS connectivity lands there, not in Orange County. CAPACITY, added 2026-09-21 and Low by design: Centersquare publishes specs for this site on its own website, which is where the figure comes from — utility power 24.9MW, UPS capacity 9.2MW, generator capacity 16MW, 73,952 sq ft raised floor. Centersquare discloses three different figures per site — utility power (the building's incoming grid service), UPS capacity (conditioned power actually deliverable to racks) and generator capacity (backup). UPS is used as capacityMW here. That is NOT the aws-santaclara precedent, and the difference is worth stating: Santa Clara's 20MW is a contracted 15-year power agreement to deliver 20MW to that facility, whereas Centersquare's utility figure is a service-entrance rating for a multi-tenant building, which is not a statement about the data center's capacity at all. Two checks settle it. Against raised floor the UPS figures give 124, 117 and 107 W/sq ft for LAX3, LAX4 and LAX5 — a tight band, and the expected density for this generation of colocation — while the utility figures give 337, 1,127 and 330 W/sq ft, which is neither consistent nor physically plausible. And on the utility figure LAX4 would be the largest of the three at 25.9MW despite having 22,987 sq ft of raised floor against LAX3's 73,952, which is plainly wrong. Note this is a consistency test used to CHOOSE among disclosed figures, not a capacity derived from floor area — deriving MW from square footage would be a fabrication and is not what is happening here. All three disclosed numbers are kept here so nothing is lost if the convention is revisited. This is single-class evidence — the operator's own word — so it scores Low no matter how many of its own pages repeat it, and that is the correct reading of it. Searched without a usable result, so it needn't be redone blind: SCAQMD's FIND database is the right channel and remains the one thing that would lift this past Low, but its facility-detail tool at xappprod.aqmd.gov/find is a JavaScript single-page app that returns only a loading shell to a plain fetch, and no server-side query endpoint was found. A facility ID of 119219 is indexed for what appears to be Evoque Hawthorne and is the thread to pull with a browser that executes JS. On EDGAR: Cyxtera's 2021 and 2022 10-Ks do not full-text match any of these addresses; a 2021 Sila Realty Trust 8-K confirms the Hawthorne address and Evoque as tenant but carries no MW; and a 2026 Csquare Inc. S-1 (CIK 2105398, the securitization vehicle) names 2681 Kelvin Avenue LLC and 2301 West 120th Street among its asset entities and discloses roughly 389MW of sellable power capacity across 64 sites — R-class, real, and deliberately NOT used, because a portfolio total cannot stand in for a site figure. Directory listings continue to disagree with each other and with the operator (7.2, 12 and 20MW have all been published for LAX3 alone); they are class D and carry no weight however many agree."
  },
  {
    ...
    provenance: {
      capacityMW: {
        ...
        asOf: "2026-09-21",
        evidence: [
          { source: 2, cls: "P", note: "Centersquare's live LA market page, LAX4 entry: utility power 25.9MW, UPS capacity 2.7MW, generator capacity 4MW, 22,987 sq ft raised floor. The 2.7MW UPS figure is used — see notes. No separate PDF spec sheet covers this site; the LA-market PDF carries only LAX3 and LAX5." }
        ],
        conflicting: false
      },
      status: {
  ...
  },
  {
    ...
    provenance: {
      capacityMW: {
        ...
        asOf: "2026-09-21",
        evidence: [
          { source: 2, cls: "P", note: "Centersquare's own LA spec sheet for LAX5: utility power 17.3MW, UPS capacity 5.6MW, generator capacity 12MW, 52,470 sq ft raised floor. The 5.6MW UPS figure is used — see notes for why, and note the other two are recorded rather than discarded." },
          { source: 3, cls: "P", note: "The same figures on Centersquare's live Los Angeles market page, fetched separately. Same operator and same class, so this corroborates transcription rather than the claim — it is one class however many documents agree." }
        ],
        conflicting: false
      },
